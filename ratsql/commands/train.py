        ...
        with mlflow.start_run() as run:
            with self.init_random:
                ...
                if config['optimizer'].get('name', None) == 'bertAdamw':
                    ...
                    assert len(non_bert_params) + \
                        len(bert_params) == len(list(self.model.parameters()))

                    optimizer = registry.construct(
                        'optimizer', config['optimizer'], non_bert_params=non_bert_params,
            ...
            if 'pretrain' in config and last_step == 0:
                ...
                # for evaluating pretrained models
                ...
                for batch in train_data_loader:
                    ...
                    if last_step % self.train_config.eval_every_n == 0:
                        if self.train_config.eval_on_train:
                            self._eval_model(
                                self.logger, self.model, last_step, train_eval_data_loader, 'train',
                                num_eval_items=self.train_config.num_eval_items,
                            )
                        if self.train_config.eval_on_val:
                            self._eval_model(
                                self.logger, self.model, last_step, val_data_loader, 'val',
                                num_eval_items=self.train_config.num_eval_items,
                            )
...
def main(args):
    ...
    with open(
        os.path.join(
            args.logdir,
            f'config-{datetime.datetime.now().strftime("%Y%m%dT%H%M%S%Z")}.json',
        ), 'w',
    ) as f:
